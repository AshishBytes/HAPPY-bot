exports.run = async (client, message, args) => {
  ...
  if (deleteCount > 100) {
    ...
    message.inlineReply('Can handle only 100 messages. Deleting 100 messages').then(e => setTimeout(() => e.delete(), 2000))
  }
  ...
  try {
    ...
    // Handle error
    message.inlineReply(`Couldn't delete messages because of: ${ error }`)
  ...
  // Notify if success
  message.inlineReply(`Successfully deleted ${ deleteCount } ${ deleteCount === 1 ? 'message' : 'messages' }.`).then(el => setTimeout(() => el.delete(), 3000))
  return 0
}
exports.help = {
  name: '`purge`',
